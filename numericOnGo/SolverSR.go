...
func (p *Solver) SolveLayer(iter int) {
	...
	for j := 1; j < p.ny-1; j++ {
		//fill coeff
		for i := 1; i < p.nx-1; i++ {
			FF[i-1] = -(1-p.alfa)*(p.a/p.hx)*(p.a/p.hx)*temp[i+1][j] + (2*p.a*p.a*(1-p.alfa)/(p.hx*p.hx)-1/p.tau)*temp[i][j] - (1-p.alfa)*(p.a/p.hy)*(p.a/p.hx)*temp[i-1][j]
			AA[i-1] = p.alfa * (p.a / p.hx) * (p.a / p.hx)
			BB[i-1] = p.alfa * (p.a / p.hx) * (p.a / p.hx)
			CC[i-1] = -2 * p.alfa * p.a*p.a/(p.hx*p.hx) - 1/p.tau
	...
	for i := 1; i < p.nx-1; i++ {
		//fill coeff
		for j := 1; j < p.ny-1; j++ {
			FF[j-1] = -(1-p.alfa)*(p.a/p.hy)*(p.a/p.hy)*temp[i][j+1] + (2*p.a*p.a*(1-p.alfa)/(p.hy*p.hy)-1/p.tau)*temp[i][j] - (1-p.alfa)*(p.a/p.hy)*(p.a/p.hy)*temp[i][j-1]
			AA[j-1] = p.alfa * (p.a / p.hy) * (p.a / p.hy)
			BB[j-1] = p.alfa * (p.a / p.hy) * (p.a / p.hy)
			CC[j-1] = -2 * p.alfa * p.a*p.a/(p.hy*p.hy) - 1/p.tau
